...
class Method():
    ...
    def mergeSameSheet(file_name, sheet_name, path, **kwargs) -> pd.DataFrame:
        ...
        for file in file_list:
            file_path = path + "/" + file
            return_sheet=pd.concat([return_sheet, pd.read_excel(file_path, header=kwargs["header"], index_col=kwargs["index_col"], sheet_name=sheet_name)],ignore_index =False)
            return_sheet.groupby(return_sheet.index).first() 
        return return_sheet
